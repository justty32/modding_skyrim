...
- **本資料夾不是 git repo**（使用者決定 2026-07-17）：只有 `projects/` 下各子專案自己是獨立 git repo——原本三個（ModForge、houseCARL fork、my_skyrim_plugin_1），2026-08-02 又從 ModForge `sub_projs/` 抽出八個：godot-worldspace-editor、scene-capture-bridge、model-converter、agent-bridge、darksouls-port、sofia-patch、skyrim-voicegen、game-data（**共 11 個**；抽出的都未帶舊 commit 歷史，ModForge 原位置留 stub 導引）。同日另有三份**純文檔**子專案搬進 `analysis/`（mod-survey、tool-survey、followers-patch），那三份**不是 git repo**。跨 repo 連結假設各 repo **同層 clone 在 `projects/` 下**。不要在 `~/repo/moddings/skyrim` 執行 `git init`/commit；`analysis/` 等文件直接改檔即可。houseCARL 原始碼來自 `https://github.com/Avick3110/houseCARL`；`analysis/` 內文件已含結論與關鍵行號引用，不強制依賴原始碼在場。
