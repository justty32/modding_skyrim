...
- **本工作區自 2026-08-03 起是 public 母 git repo**（`justty32/modding_skyrim`），`projects/` 下 10 個獨立 repo 以 submodule 管理：ModForge、my_skyrim_plugin_1、godot-worldspace-editor、scene-capture-bridge、model-converter、agent-bridge、darksouls-port、sofia-patch、skyrim-voicegen、game-data。跨 repo 連結假設各 repo **同層 clone 在 `projects/` 下**。`projects/houseCARL` 是第 11 個本地獨立 repo，但 fix branch 還沒有可供他人 clone 的 remote commit，因此目前被 `.gitignore` 排除、**不是 submodule**；不要在母 repo 追蹤它的內容。（**2026-08-11 已定案要改**：force-push 到 fork 後改釘 fork branch，屆時本條連同 `.gitignore` 一併更新。決策見 [fork-maintenance-decision.md](analysis/houseCARL/answers/fork-maintenance-decision.md)，執行步驟見 [WAIT_USER.md](WAIT_USER.md)，**在那之前本條仍然有效**。）另有三份純文檔子專案在 `analysis/`（mod-survey、tool-survey、followers-patch），不是獨立 repo。houseCARL 原始碼來自 `https://github.com/Avick3110/houseCARL`；`analysis/` 內文件已含結論與關鍵行號引用，不強制依賴原始碼在場。
