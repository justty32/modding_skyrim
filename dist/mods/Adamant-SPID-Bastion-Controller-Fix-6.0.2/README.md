# Adamant 6.0.2 SPID Bastion Controller Fix

Adamant 6.0.2 的最小化 SPID 設定修正。

官方 MAIN 檔的 `Adamant_DISTR.ini` 要把 NPC 版 Bastion controller perk 分發給演員，
但設定使用不存在的 EditorID `MAG_BastionNPC`。官方 `Adamant.esp` 6.0.2 實際提供的記錄是
...
- 需要 Adamant 6.0.2 MAIN 與其正式依賴。
- 在 MO2 左側讓本包高於 Adamant 本體，使本包的 `Adamant_DISTR.ini` 成為 VFS winner。
- 不需要 Blade and Blunt addon；這筆 NPC controller perk 位於 Adamant 6.0.2 MAIN。
