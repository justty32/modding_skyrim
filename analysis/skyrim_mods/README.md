...
分析對象：`~/skyrim_mods/` 下已解壓的 Skyrim SE mod。它們是 **ModForge**（`projects/ModForge`，一個用 Mutagen 程式化生成 Skyrim plugin 的工具）的**參考素材**——不是要改它們，而是拆解它們「怎麼做到」，回饋給 ModForge 的 spec / builder 設計。
