...
把七個參考 mod 對照 **ModForge**（`projects/ModForge`，程式化生成 Skyrim plugin 的工具）目前的能力，整理出「已對齊 / 可短期補 / 範式級缺口」三層。本檔是綜合層，個別細節見各 `architecture/*.md` 與 `details/dialogue-targeting-technique.md`。
