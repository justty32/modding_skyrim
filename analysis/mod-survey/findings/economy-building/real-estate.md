...
| 4 個 relationship PERK ±rank | PERK（含 entry-point，[perk-entry-points](../systems-encounters/perk-entry-points.md)）+ RELA | ✅ landed |
...
| MCM（買賣 / 倍率 / 週期 / 開關 / cheat 頁）| **MCM 生成（`McmGen.cs`/`Generator.Build.Mcm.cs`，`Spec.Mcm.cs`）** | ✅ landed，**但生成的是 MCM-Helper `config.json` 風格**，非 RE 的 `SKI_ConfigBase` 手寫腳本（兩者最終都出 SkyUI MCM，功能等價；ModForge 走宣告式那條，見 [mcm-helper](../frameworks-tools/mcm-helper.md)）|
