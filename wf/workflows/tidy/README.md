...
4. **考慮使用者——給人點的留 md、給 AI 讀的進資料檔**：同質記錄表（每列一筆記錄）> 1 KB 就抽成 `.json`／`.csv`（存取統一走 `tabledb.py`）；連結表不看 bytes，超過**十條**才考慮，給人導航的（路由表、目錄表、派發表）留 md，給 AI 消化的（候選表、ledger 裡的連結欄）才抽資料檔。**整檔 > 8 KB 就按語意拆資料夾**——這兩個門檻各自獨立觸發，格式契約與 A／B 拆法在 [common/data-files.md](../common/data-files.md)，不在這裡重述。掃描：`wf/tools/find_big_lists.py`、`find … -size +8192c`。
