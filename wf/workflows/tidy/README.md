...
<!-- wf-nav -->
1. **過時的就封存，把指向它的連結清乾淨，當做它不存在**：活文件裡的連結是**拿掉**（改純文字或整句刪），不是改指到 `archive/` 路徑；只有 `archive/README.md` 的索引表可以連過去。「過時」的判準：文件自述歷史／被取代／前提作廢；文件描述的目標（計畫、批次、profile／環境配置）已不存在；結論已整份被某現役文件吸收。
