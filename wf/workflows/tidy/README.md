...
4. **條列式區塊 > 1 KB 就抽成 `.json`／`.csv`**（存取統一走 `tools/tabledb.py`），**整檔 > 8 KB 就按語意拆資料夾**——兩個門檻各自觸發，做法與例外在 [splitting.md](splitting.md)。掃描：`tools/find_big_lists.py`、`find … -size +8192c`。
...
- `python3 tools/find_big_lists.py <repo>` 只剩導航表；`find <repo> -name '*.md' -not -path '*/archive/*' -size +8192c | wc -l` → 0。
- 報告附 `moves.tsv`（`舊路徑<TAB>新路徑`）。

## 流程

角色分工（2026-08-30 定）：**主 session（Fable）**只盤點、寫交接書、核驗、修跨 repo 連結、commit；**一個 repo 一條 Fable 管理線**（`Agent` `model: fable`），它分類、切批、派工人、逐批驗收；**工人預設 Sonnet**（表格抽資料檔、對帳、連結修正這類機械活），要語意判斷的批（散文拆段、封存判定）才用 Opus。線**不得進別的 repo**；管理線簡報範本見 [manager-brief.md](manager-brief.md)。
...
| 檔案 | 內容 |
|------|------|
| [splitting.md](splitting.md) | >8 KB 拆檔的 A／B 兩法：同質大表抽成 `.json`／`.csv`＋`tools/tabledb.py` 統一 CRUD、散文拆資料夾；原路徑一律保留 |
| [manager-brief.md](manager-brief.md) | Fable 管理線的角色簡報：怎麼切批、派 Sonnet／Opus、逐批驗收、交棒 |
